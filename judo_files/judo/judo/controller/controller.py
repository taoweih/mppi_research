        ...
        while i < self.max_opt_iters and not self.optimizer.stop_cond():
            ...
            if isinstance(self.optimizer, MPPIStagedRollout):
                ...
                for n in range(num_stages-1):
                    ...
                    self.states[:,n*timesteps_per_stage:(n+1)*timesteps_per_stage,:] = states_partial_rollout
                    self.sensors[:,n*timesteps_per_stage:(n+1)*timesteps_per_stage,:] = sensors_partial_rollout
                    latest_states = states_partial_rollout[:,-1,:]

                    
                    # resampling indices
                    kde = KernelDensity(bandwidth=self.optimizer.kde_bandwidth, kernel="gaussian")
                    kde.fit(latest_states)

                    score = kde.score_samples(latest_states)
                    p_x = np.exp(score)
                    inv_px = (1.0 / p_x+1e-5)**1.2
                    inv_px = inv_px / inv_px.sum()

                    indices = np.random.choice(len(inv_px), size=self.optimizer.num_rollouts, p = inv_px, replace=True)

                    # reorder things around (only need to reorder up to current steps but won't matter since the later ones will be overwritten)
                    curr_state = latest_states[indices]
                    self.states = self.states[indices]
                    self.sensors = self.sensors[indices]
                    self.candidate_knots = self.candidate_knots[indices]
                    candidate_knots_normalized = candidate_knots_normalized[indices]

                    # sample new knots
                    new_partial_control_knots_normalized = self.optimizer.sample_partial_control_knots(nominal_knots_normalized[(n+1)*nodes_per_stage:,:])
                    ...
                    # update candidate knots
                    candidate_knots_normalized[:,(n+1)*nodes_per_stage:,:] = new_partial_control_knots_normalized
                    self.candidate_knots[:,(n+1)*nodes_per_stage:,:] = new_partial_control_knots

                    ### update rollout_controls using new samples
                    partial_new_time = curr_time+np.linspace(self.task.dt*(n+1)*timesteps_per_stage, self.horizon, self.optimizer_cfg.num_nodes-(n+1)*nodes_per_stage, endpoint=True)
                    new_partial_splines = make_spline(partial_new_time, new_partial_control_knots, self.spline_order)

                    remaining_rollout_time = self.task.dt * np.arange((n+1)*timesteps_per_stage,self.num_timesteps)
                    self.rollout_controls[:,(n+1)*timesteps_per_stage:,:]= new_partial_splines(curr_time + remaining_rollout_time)

                # rollout remaining control
                partial_rollout_control = self.rollout_controls[:,(num_stages-1)*timesteps_per_stage:,:]
                states_partial_rollout, sensors_partial_rollout = self.rollout_backend.rollout(self.model_data_pairs, curr_state, partial_rollout_control,is_full_state=True)
                self.states[:,(num_stages-1)*timesteps_per_stage:,:] = states_partial_rollout
                self.sensors[:,(num_stages-1)*timesteps_per_stage:,:] = sensors_partial_rollout

                
                # need to update
                # self.states, self.sensors
                #   reorder after each stage
                # candidate_knots_normalized, candidate_knots
